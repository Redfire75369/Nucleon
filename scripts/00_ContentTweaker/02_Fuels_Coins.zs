...
for tier in tierList {
	MaterialSystem.getPartBuilder().setName(tier).setPartType(mods.contenttweaker.MaterialSystem.getPartType("item")).build();
	MaterialPart.getPart(tier).hasOverlay(true);
}

for variant in variantList {
	val mat = MaterialSystem.getMaterialBuilder().setName(variant).build();
